...
reviewer's memory. That state is not hypothetical here. ``cogno-praxis`` carried a bookkeeper
suite in exactly it until #90, and a sweep on 2026-09-02 found this repo holding SIX files and
56 node ids the same way — all of ``tests/integration`` except ``test_noumeno.py``, including
the five that pin the SUPEREGO judge fail-CLOSED and the two that run a whole turn end to end.
...
workflow keeps passing and the new directory runs nowhere. Fifteen of the eighteen repos in
this ecosystem are one new folder away from it. ``cogno-engram`` is immune by construction — a
BARE ``pytest``, scoped by ``testpaths``, names no child at all — but immunity that lives in
the current text of a file is a rule someone has to remember. This is the rule that runs.
...
* a CONDITIONAL job still counts as an invocation. The nightly below is gated on
  ``schedule || workflow_dispatch`` on purpose — assertions against a live model must not
  decide whether main is green — so "only nightly" is a real answer, not a violation. WHICH
  paths are nightly-only is a second, weaker, separate assertion at the bottom of this file;
...
def _strip_heredocs(text: str) -> str:
    """Drop ``<<EOF ... EOF`` bodies. The nightly below embeds a python probe whose lines are
    not shell at all; lexing them is noise at best and an unbalanced quote at worst."""
    out: list[str] = []
...
def test_the_lexer_reads_the_forms_this_repo_actually_uses():
    cases = {
        "pytest tests/unit -q --cov=pkg --cov-report=term-missing --cov-fail-under=85":
            ["tests/unit"],
        'pytest tests/integration -q -m "not perception and not semantics"':
            ["tests/integration"],
        "pytest tests/integration -q || [ $? -eq 5 ]": ["tests/integration"],
        "pytest tests/integration/test_noumeno.py -q": ["tests/integration/test_noumeno.py"],
        "python -m pytest tests -q": ["tests"],
        "pytest ./tests/unit/ -q": ["tests/unit"],
        "pytest tests/unit/test_x.py::TestC::test_m": ["tests/unit/test_x.py"],
...
def test_a_neighbouring_command_does_not_donate_its_arguments():
    """``ruff check ... tests`` on the next line is not a pytest path, and ``[ $? -eq 5 ]``
    after a ``||`` is not one either. Either would silently widen what looks invoked."""
    block = "pytest tests/unit -q\nruff check cogno_anima cognobench tests scripts\nmypy"
...
def test_a_heredoc_body_is_not_shell():
    """The nightly's python probe sits in a ``<<'EOF'`` block, apostrophes and all."""
    block = ("python - <<'EOF'\n"
...
def test_the_invariant_would_notice_an_uninvoked_file(pytestconfig):
    ...
    assert _covers("tests/integration", "tests/integration/test_ego.py")
    assert _covers(".", "tests/integration/test_ego.py")
...
def test_which_paths_run_only_on_a_conditional_job_is_a_decision_not_a_drift(pytestconfig):
    ...
    pull-request gate for everything. A path reached only by the nightly IS invoked, and that
    is right wherever the assertions depend on a live model: sampling a real model must not
