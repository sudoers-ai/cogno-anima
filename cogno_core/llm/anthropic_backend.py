"""
cogno_core.llm.anthropic_backend — Anthropic Claude LLM backend (Messages API).
...
Optional dependency: ``pip install "cogno-core[anthropic]"`` (or ``anthropic``).
"""
...
logger = logging.getLogger("cogno_core.llm.anthropic")
...
class AnthropicBackend:
    ...
    def _client(self):
        try:
            ...
            raise ImportError(
                'anthropic not installed. Run: pip install "cogno-core[anthropic]"'
            ) from exc
