"""
cogno_core.llm.gemini_backend — Google Gemini LLM backend (google-genai SDK).
...
Optional dependency: ``pip install "cogno-core[gemini]"`` (or ``google-genai``).
"""
...
logger = logging.getLogger("cogno_core.llm.gemini")
...
class GeminiBackend:
    ...
    def _client(self):
        try:
            ...
            raise ImportError(
                'google-genai not installed. Run: pip install "cogno-core[gemini]"'
            ) from exc
