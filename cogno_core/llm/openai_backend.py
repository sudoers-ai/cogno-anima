"""
cogno_core.llm.openai_backend — OpenAI-compatible LLM backend.
...
tenant contextvar (the host owns key rotation), and — per cogno-core's
...
Optional dependency: ``pip install "cogno-core[openai]"`` (or ``openai``).
"""
...
logger = logging.getLogger("cogno_core.llm.openai")
