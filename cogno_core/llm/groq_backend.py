"""
cogno_core.llm.groq_backend — Groq LLM backend (ultra-fast OpenAI-compatible API).
...
Optional dependency: ``pip install "cogno-core[groq]"`` (or ``groq``).
"""
...
from cogno_core.llm.openai_backend import _openai_tool_call, _safe_close

logger = logging.getLogger("cogno_core.llm.groq")
...
class GroqBackend:
    ...
    def _client(self):
        try:
            ...
            raise ImportError('groq not installed. Run: pip install "cogno-core[groq]"') from exc
