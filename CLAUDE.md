...
5. **SUPEREGO** (`cogno_anima/stages/superego.py`, class `SuperegoStage`) — guardrails, judge & voicer:
   - **EGO=executor, SUPEREGO=locutor:** the SUPEREGO **writes** the final user response from the EGO's gathered data (it does NOT review a pre-written one — that is the key divergence from the parent). Three LLM ops, **A2** (host injects a backend per op, may differ — e.g. big judge + small voicer, or the same model):
     ...
     - `evaluate(ctx, backend, *, limits_prompt) -> SuperegoResult` — the **JUDGE**: approve the EGO execution or return a `critique`. **Criterion #1 is goal↔execution** ("asked X, did X not Y"), then **#2 user constraints** (honored every `intent.constraints`, did NOT do what `intent.negation` forbade — rendered into the prompt via `_format_restrictions`), then completeness, grounding, safety/limits. **Fail-CLOSED** (never approve unverified — a false-pass is worse than a retry). The `critique` feeds the EGO's `ctx.metadata["ego_correction"].reason` (the correction loop closes).
     - `voice(ctx, backend, *, voice_prompt) -> SuperegoResult` — **writes** `response` in the persona's voice **and limits** (limits go to the voice too, not only the judge), grounded in the tool data (exact figures verbatim); `strip_cot`; runs a **deterministic PII backstop on the OUTPUT** (flags `pii:flagged_in_output`, does NOT auto-redact — avoids over-redaction); feeds **lexical** `compute_synthesis` drift + recomputes cumulative. Also surfaces a **register-accommodation** signal (`# Register`, derived from NER `parole`): match the user's formality *where it does not conflict with the persona* — **the persona voice/limits always win**. Raises on backend failure (errors propagate).
